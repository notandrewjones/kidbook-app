...
async function handler(req, res) {
  ...
  try {
    // Get ALL orders for this user (not just paid - show refunded, cancelled, etc.)
    const { data: orders, error } = await supabase
      .from("orders")
      .select("*")
      .eq("user_id", user.id)
      .in("status", ["paid", "refunded"]) // Show paid and refunded orders (not pending/failed/expired)
    ...
    const formattedOrders = orders.map(order => {
      ...
      return {
        ...
        cancellationReason: order.cancellation_requested_reason || null,
      };
    });
